(function () {
  ...
  function ensureIds(cats) {
    var cs = {}; cats.forEach(function (c) {
      if (!c.id) { var b = slug(c.name && c.name.fr) || 'cat', id = b, k = 2; while (cs[id]) id = b + '-' + (k++); c.id = id; }
      cs[c.id] = 1; var vs = {};
      (c.verses || []).forEach(function (v) { if (!v.id) { var b = slug(v.fr && v.fr.ref) || 'v', id = b, k = 2; while (vs[id]) id = b + '-' + (k++); v.id = id; } vs[v.id] = 1; });
    });
  }
  function nettoie(cats) { return cats.map(function (c) { return { id: c.id, name: { fr: (c.name || {}).fr || '', en: (c.name || {}).en || '' }, verses: (c.verses || []).map(function (v) { return { id: v.id, fr: { text: (v.fr || {}).text || '', ref: (v.fr || {}).ref || '' }, en: { text: (v.en || {}).text || '', ref: (v.en || {}).ref || '' } }; }) }; }); }

  var built = false, W = null, dragRow = null;
  ...
  function construire() {
    ...
    mk('div', panel, 'font-family:sans-serif;font-size:12px;color:#9a9a9a;margin:0 0 14px').textContent = FR ? 'Base commune à tous les comptes. Glisse un verset pour le réordonner ou le changer de catégorie ; ↑↓ pour les catégories. Ne touche pas aux identifiants existants.' : 'Shared base. Drag a verse to reorder it or move it between categories; ↑↓ for categories.';
    ...
    function qsa(sel, r) { return Array.prototype.slice.call((r || host).querySelectorAll(sel)); }
    function posApres(body, y) { var rows = qsa('.lv-mem-vrow', body).filter(function (r) { return r !== dragRow; }); var best = null, bo = -Infinity; rows.forEach(function (r) { var b = r.getBoundingClientRect(); var off = y - b.top - b.height / 2; if (off < 0 && off > bo) { bo = off; best = r; } }); return best; }
    function syncFromDOM() { qsa('.lv-mem-body').forEach(function (body) { if (body.__c) body.__c.verses = qsa('.lv-mem-vrow', body).map(function (r) { return r.__v; }); }); }
    ...
    function rendre() {
      ...
      W.forEach(function (c) {
        ...
        var body = mk('div', block, 'padding:10px;min-height:30px;' + (c.__open ? '' : 'display:none')); body.className = 'lv-mem-body'; body.__c = c;
        body.addEventListener('dragover', function (e) { if (!dragRow) return; e.preventDefault(); e.dataTransfer.dropEffect = 'move'; var a = posApres(body, e.clientY); if (a == null) body.insertBefore(dragRow, body.querySelector('.lv-mem-addv')); else body.insertBefore(dragRow, a); });
        body.addEventListener('drop', function (e) { if (!dragRow) return; e.preventDefault(); });
        function fillBody() {
          body.innerHTML = '';
          c.verses.forEach(function (v) {
            v.fr = v.fr || { text: '', ref: '' }; v.en = v.en || { text: '', ref: '' };
            var r = mk('div', body, 'border-left:2px solid #333;padding:8px 0 8px 10px;margin:8px 0;background:#0c0c0c'); r.className = 'lv-mem-vrow'; r.__v = v; r.setAttribute('draggable', 'true');
            r.addEventListener('dragstart', function (e) { dragRow = r; e.dataTransfer.effectAllowed = 'move'; try { e.dataTransfer.setData('text/plain', ''); } catch (_) {} setTimeout(function () { r.style.opacity = '.35'; }, 0); });
            r.addEventListener('dragend', function () { r.style.opacity = ''; var d = dragRow; dragRow = null; if (d) { syncFromDOM(); rendre(); } });
            var bar = mk('div', r, 'display:flex;gap:6px;align-items:center;margin-bottom:5px;flex-wrap:wrap');
            mk('span', bar, 'cursor:grab;color:#888;font-family:sans-serif;font-size:14px;user-select:none').textContent = '⠿';
          ...
          var addV = mk('span', body, 'display:inline-block;margin-top:8px;background:#000;color:#cfc9bb;border:1px solid #444;padding:8px 12px;cursor:pointer;font-family:sans-serif;font-size:13px'); addV.className = 'lv-mem-addv'; addV.textContent = FR ? '+ Ajouter un verset' : '+ Add a verse';
    ...
    save.addEventListener('click', function () {
      ensureIds(W); var clean = nettoie(W); stat.textContent = FR ? 'Enregistrement…' : 'Saving…';
      DOC.set({ categories: clean }).then(function () { var p = P(); if (p) { p.length = 0; clean.forEach(function (c) { p.push(JSON.parse(JSON.stringify(c))); }); } if (typeof renderCats === 'function') { try { renderCats(); } catch (e) {} } stat.textContent = FR ? 'Enregistré. Base commune mise à jour.' : 'Saved. Shared base updated.'; }).catch(function (e) { stat.textContent = (FR ? 'Erreur : ' : 'Error: ') + e.message; });
    });
  }
})();
