...
const APPARENCE_JS = fs.existsSync('apparence.js') ? fs.readFileSync('apparence.js', 'utf8') : '';
const MEMOADMIN_JS = fs.existsSync('memoriser-admin.js') ? fs.readFileSync('memoriser-admin.js', 'utf8') : '';
const CONTENU_JS = fs.existsSync('contenu.js') ? fs.readFileSync('contenu.js', 'utf8') : '';
const THEMES_JS = fs.existsSync('themes.js') ? fs.readFileSync('themes.js', 'utf8') : '';
const ACCUEIL_JS = fs.existsSync('accueil.js') ? fs.readFileSync('accueil.js', 'utf8') : '';
const EXPORT_JS = fs.existsSync('export.js') ? fs.readFileSync('export.js', 'utf8') : '';
...
function mainAccueil(lang, base) {
  ...
  return `<div class="vue">
    ...
    <div class="memo-bloc">
      ...
      <a class="memo-start" href="/memoriser.html" data-lv-txt="memo_start">${u.memo_start}</a>
    </div>
...
if (fs.existsSync('memoriser.html')) {
  ...
  if (APPARENCE_JS && mh.indexOf('</body>') >= 0) {
    mh = mh.replace('</body>', '<script>' + APPARENCE_JS + '</script></body>');
  }
  if (MEMOADMIN_JS && mh.indexOf('</body>') >= 0) {
    mh = mh.replace('</body>', '<script>' + MEMOADMIN_JS + '</script></body>');
  }
  if (EXPORT_JS && mh.indexOf('</body>') >= 0) {
    mh = mh.replace('</body>', '<script>' + EXPORT_JS + '</script></body>');
  }
