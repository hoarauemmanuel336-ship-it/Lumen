...
(function lvAdminBoot() {
  ...
  function ttsStyles() {
    ...
    st.textContent = '.tts-voile{position:fixed;inset:0;background:rgba(4,4,4,.88);backdrop-filter:blur(2px);-webkit-backdrop-filter:blur(2px);z-index:220;display:flex;align-items:center;justify-content:center;padding:20px;animation:ttsV .25s ease both}'
      + '.tts-boite{background:#000;box-shadow:inset 0 0 0 1px var(--filet-fort,rgba(231,224,207,.16)),inset 0 1px 0 rgba(231,224,207,.05);clip-path:polygon(0 0,calc(100% - 14px) 0,100% 14px,100% 100%,0 100%);animation:ttsB .3s cubic-bezier(.2,.7,.3,1) both;max-width:480px;width:100%;padding:34px 32px 26px;position:relative;max-height:90vh;overflow-y:auto;overflow-x:hidden;box-sizing:border-box;scrollbar-width:thin;scrollbar-color:var(--filet-fort,rgba(231,224,207,.35)) #000}'
      + '.tts-boite::-webkit-scrollbar{width:10px}'
      + '.tts-boite::-webkit-scrollbar-track{background:#000}'
      + '.tts-boite::-webkit-scrollbar-thumb{background:var(--filet-fort,rgba(231,224,207,.35))}'
      + '.tts-boite::-webkit-scrollbar-thumb:hover{background:var(--filet-fort,rgba(231,224,207,.6))}'
      + '.tts-x{position:absolute;top:12px;right:16px;background:none;border:none;color:rgba(231,224,207,.5);font-size:19px;cursor:pointer;padding:6px 9px;transition:color .3s}'
      + '.tts-x:hover{color:var(--or-pale,#f8f3e6)}'
      + '.tts-t{font-family:"Cormorant Garamond",serif;font-weight:400;font-size:27px;letter-spacing:.02em;color:#fff;margin:0 0 7px}'
      + '.tts-p{font-size:13px;line-height:1.6;color:rgba(231,224,207,.55);margin:0}'
      ...
      + '.tts-filet{height:1px;background:linear-gradient(90deg,var(--filet-fort,rgba(231,224,207,.55)),var(--filet-fort,rgba(231,224,207,.22)) 38%,rgba(231,224,207,.07) 72%,transparent);margin:9px 0 18px}'
      ...
      + '.tts-in{width:100%;box-sizing:border-box;background:var(--encre-2,#0b0b0b);border:none;box-shadow:inset 0 0 0 1px var(--filet-fort,rgba(231,224,207,.2));color:#fff;padding:11px 14px;font-size:15px;font-family:inherit;border-radius:0;transition:box-shadow .3s}'
      + '.tts-in:focus{outline:none;box-shadow:inset 0 0 0 1px var(--or,#efe6cf)}'
      + '.tts-sel{position:relative}'
      + '.tts-sel::after{content:"\u203A";position:absolute;right:14px;top:50%;transform:translateY(-50%) rotate(90deg);color:var(--filet-fort,rgba(231,224,207,.6));pointer-events:none;font-size:16px;line-height:1}'
      + 'select.tts-in{appearance:none;-webkit-appearance:none;cursor:pointer;padding-right:36px}'
      + '.tts-chk{display:flex;gap:10px;align-items:center;margin-top:18px;font-size:13.5px;color:rgba(231,224,207,.75);cursor:pointer}'
      + '.tts-chk input{accent-color:var(--or,#efe6cf)}'
      + '.tts-acts{display:flex;gap:14px;flex-wrap:wrap;align-items:stretch;margin-top:24px}'
      + '.tts-acts .tts-btn{flex:1 1 170px}'
      + '.tts-acts .tts-btn2{flex:1 1 150px}'
      ...
      + '.tts-btn2{font-family:"Cormorant Garamond",serif;font-size:13px;letter-spacing:.1em;text-transform:uppercase;background:none;border:none;box-shadow:inset 0 0 0 1px var(--filet-fort,rgba(231,224,207,.25));color:rgba(231,224,207,.65);padding:12px 18px;cursor:pointer;transition:box-shadow .3s,color .3s}'
      + '.tts-btn2:hover{box-shadow:inset 0 0 0 1px var(--filet-fort,rgba(231,224,207,.7));color:var(--or-pale,#f8f3e6)}'
      + '.tts-mini{background:none;border:none;font-family:"Cormorant Garamond",serif;font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:rgba(231,224,207,.55);cursor:pointer;padding:0;transition:color .3s}'
      + '.tts-mini:hover{color:var(--or,#efe6cf)}'
      + '.tts-etat{font-size:12.5px;color:rgba(231,224,207,.55);margin:0 0 12px}'
      + '.tts-ver{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:3px 16px;align-items:center;padding:13px 16px;margin-bottom:10px;background:var(--encre-2,#0b0b0b);box-shadow:inset 0 0 0 .5px var(--filet,rgba(231,224,207,.14)),inset 0 1px 0 rgba(231,224,207,.05);clip-path:polygon(0 0,calc(100% - 10px) 0,100% 10px,100% 100%,0 100%);transition:box-shadow .3s}'
      + '.tts-ver:hover{box-shadow:inset 0 0 0 .5px var(--filet-fort,rgba(231,224,207,.3)),inset 0 1px 0 rgba(231,224,207,.06)}'
      ...
      + '.tts-ver-m{grid-column:1;font-size:12px;line-height:1.5;color:rgba(231,224,207,.55)}'
      + '.tts-ver-a{grid-column:2;grid-row:1/span 2;display:flex;flex-direction:column;align-items:flex-end;gap:9px}'
      + '.tts-note{font-size:11.5px;line-height:1.6;color:rgba(231,224,207,.45);margin:24px 0 0}'
      + '.tts-a{color:var(--or,#efe6cf);text-decoration:none}.tts-a:hover{text-decoration:underline}'
      + '.tts-toast{position:fixed;left:50%;bottom:26px;transform:translateX(-50%) translateY(12px);background:#000;box-shadow:inset 0 0 0 1px var(--filet-fort,rgba(231,224,207,.3));color:#fff;padding:11px 18px;font-size:13.5px;z-index:240;opacity:0;pointer-events:none;transition:opacity .3s,transform .3s;max-width:86vw;text-align:center}'
      + '.tts-toast.vu{opacity:1;transform:translateX(-50%) translateY(0)}'
      + '.tts-btn:focus-visible,.tts-btn2:focus-visible,.tts-mini:focus-visible,.tts-x:focus-visible{outline:1px solid var(--or,#efe6cf);outline-offset:2px}'
      + '@keyframes ttsV{from{opacity:0}to{opacity:1}}'
      + '@keyframes ttsB{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:none}}'
      + '@media (prefers-reduced-motion:reduce){.tts-voile,.tts-boite{animation:none}}';
  ...
  function ttsModale(opts) {
    ...
    b.innerHTML = '<button type="button" class="tts-x" aria-label="' + T('Fermer', 'Close') + '">\u2715</button>'
      ...
      + '<div style="margin-top:8px;text-align:center"><button type="button" class="tts-mini" id="tts-lv">' + T('Charger les voix de mon compte', 'Load my account\u2019s voices') + '</button></div>'
      ...
      + '<div style="margin-top:12px;text-align:center"><button type="button" class="tts-mini" id="tts-vide">' + T('Vider tout le cache audio', 'Clear the whole audio cache') + '</button></div></div>'
  ...
  function outilsArticle() {
    ...
    function decoupePhrases(t) {
      ...
      (t.match(/[^.!?\u2026]+[.!?\u2026]*\s*/g) || [t]).forEach(function (ph) {
        ...
        while (ph.length > 240) { var k = ph.lastIndexOf(',', 240); if (k < 80) k = 240; out.push(ph.slice(0, k + 1)); ph = ph.slice(k + 1).trim(); }
        if (ph) out.push(ph);
      });
      return out;
    }
  ...
  function buildHub() {
    ...
    ['fr', 'en'].forEach(function (l) {
      ...
      c.addEventListener('click', function () {
        ...
        var ouvertLi = qsa('.lva-li.on', hubBody).filter(function (x) { return x.__slug; })[0];
        if (ouvertLi) hubOpt = { slug: ouvertLi.__slug };
