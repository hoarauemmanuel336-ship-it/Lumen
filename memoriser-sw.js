...
self.addEventListener('fetch', function (e) {
  ...
  e.respondWith(
    caches.open(CACHE).then(function (c) {
      return c.match(cle, { ignoreSearch: true }).then(function (enCache) {
        var reseau = fetch(e.request).then(function (r) {
          if (r && (r.ok || r.type === 'opaque')) c.put(cle, r.clone());
          return r;
        ...
        return fin.then(function (r) { return r || Response.error(); });
      });
    })
  );
});
